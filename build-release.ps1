...
$updaterCheck = Start-Process (Join-Path $publish 'LiethUpdater.exe') -ArgumentList '--self-check' -Wait -PassThru
if ($updaterCheck.ExitCode -ne 0) { throw "L'updater autonome est invalide (code $($updaterCheck.ExitCode))." }
Remove-Item -LiteralPath (Join-Path $publish 'updater') -Recurse -Force
